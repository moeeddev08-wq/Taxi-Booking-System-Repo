...
// Fix: browser "Back" button can restore this page from bfcache (a frozen
// snapshot) instead of reloading it — which would show the dashboard even
// after logout, without re-checking the session. pageshow with
// event.persisted=true fires specifically for that case, so we force a
// fresh session check whenever it happens.
